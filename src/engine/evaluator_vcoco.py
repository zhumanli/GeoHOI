...
@torch.no_grad()
def vcoco_evaluate(model, recon_model, resnet, centroids, criterion, postprocessors, data_loader, device, output_dir, thr):
    model.eval()
    criterion.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.no_grad():
            outputs = model(recon_model, resnet, centroids, samples, targets=targets)
...
def vcoco_accumulate(total_res, args, print_results, wandb_log):
    ...
    scenario1, log_res1 = vcoco_evaluator.role_eval1.evaluate(print_results)
    scenario2, log_res2 = vcoco_evaluator.role_eval2.evaluate(print_results)
    ...
    return scenario1, scenario2, log_res1, log_res2
