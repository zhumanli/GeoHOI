...
from src.maskreconstructionmodel.modelrecon.model import Model
...
def main(args):
    ...
    args.num_classes = 250 # dataset_train.num_category()
    ...
    ## reconstruction and resnest models
    recon_model = Model.load_from_checkpoint("hub/checkpoints/model_128_k32.ckpt")
    recon_model.to(device)

    resnet = models.resnet50(pretrained=True)
    resnet.to(device)
    num_features = resnet.fc.in_features
    resnet.fc = nn.Linear(num_features, 128).to(device)

    ## load cluster centroids
    centroids = torch.from_numpy(torch.load('hub/checkpoints/cluster_centroids_128.pt')).to(device)
    ...
    if args.resume:
        ...
        if args.resume.startswith('https'):
            ...
            # checkpoint = torch.load("/mnt/sda/manli/STIP/STIP-main/checkpoints/vcoco/vcoco_928a85d_best.pth", map_location='cpu')
    ...
    if args.eval:
        ...
        if args.HOIDet:
            if args.dataset_file == 'vcoco':
                checkpoint = torch.load('checkpoints/vcoco/GeoHOI_Full/vcoco/best.pth', map_location='cpu')
                model_without_ddp.load_state_dict(checkpoint['model'])
                total_res = hoi_evaluator(args, model, recon_model, resnet, centroids, criterion, postprocessors, data_loader_val, device)
                sc1, sc2, log_res1, log_res2 = hoi_accumulator(args, total_res, True, False)
                with open(os.path.join(args.output_dir, 'log_ap1.txt'), 'w') as file:
                    file.write(log_res1 + '\n')
                with open(os.path.join(args.output_dir, 'log_ap2.txt'), 'w') as file:
                    file.write(log_res2 + '\n')
            elif args.dataset_file == 'hico-det':
                test_stats = hoi_evaluator(args, model, recon_model, resnet, centroids, None, postprocessors, data_loader_val, device)
            ...
            test_stats, coco_evaluator = coco_evaluate(model, recon_model, resnet, centroids, criterion, postprocessors,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, recon_model, resnet, centroids, criterion, data_loader_train, optimizer, device, epoch, args.epochs,
        ...
        if args.validate:
            ...
            if args.dataset_file == 'vcoco':
                total_res = hoi_evaluator(args, model, recon_model, resnet, centroids, criterion, postprocessors, data_loader_val, device)
                if utils.get_rank() == 0:
                    sc1, sc2, log_res1, log_res2 = hoi_accumulator(args, total_res, True, args.wandb)
                    if sc1 > scenario1:
                        scenario1 = sc1
                        scenario2 = sc2
                        with open(os.path.join(args.output_dir, 'log_ap1.txt'), 'w') as file:
                ...
                if utils.get_rank() == 0:
                    ...
                    save_ckpt(args, model_without_ddp, optimizer, lr_scheduler, epoch, filename=str(epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_graph_feature', dest='use_graph_feature', action='store_false', default=True)
    parser.add_argument('--no_patch_feature', dest='use_patch_feature', action='store_false', default=True)
